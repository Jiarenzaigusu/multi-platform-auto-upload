...
6. 单条发布不需要填写视频绝对路径。服务器上传模式仍可用于跨设备素材和旧版助手兼容；天猫单条发布仍可选 JPG、PNG 或 WebP 自定义封面（最大 20 MiB，宽高至少 720 像素）。
7. 批量发布先把视频上传到“批量视频素材”，Excel 的“视频路径”只填写当前用户素材目录内的相对文件名。绝对路径和 `..` 跨目录路径会被拒绝。
