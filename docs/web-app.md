...
- Microsoft Edge、Patchright `BrowserRuntime`、平台 Cookie、扫码、短信、验证码、浏览器日志和失败截图都只在用户电脑上运行或保存；
- 云服务器不会创建 Edge、Browser、BrowserContext 或 Page，也不需要图形桌面、RDP 常驻会话；
- 视频会先通过 HTTPS 上传到云端任务区，再由代理下载到用户电脑，因此云服务器仍承担视频带宽和临时存储，不承担浏览器 CPU/内存负载。
...
4. 代理通过受认证的视频接口把素材下载到本机临时目录，下载和执行期间持续发送心跳；
...
单条发布视频进入 `uploads/<uuid>/`，任务进入成功、失败、取消或“结果待核对”后清理。批量素材属于用户维护的源文件，任务引用期间拒绝删除。
...
平台 Cookie、浏览器日志和失败截图不上传云端。删除云端用户不会自动擦除已离线电脑上的数据；应在用户仍可连接时从页面创建“删除本地账号”任务，或由终端管理策略清理代理目录。
...
天猫支持标题、文案、标签、活动话题、音乐、最多 6 个商品 ID、定时发布和创作者声明。京东支持标题、一个商品 ID、自主原创、定时发布和创作者声明。定时发布时间至少提前 2 小时。
...
普通终态任务自动保留 90 天或最近 2000 条，`uncertain` 任务保留到人工处理；任务日志与失败截图默认保留 30 天。
...
- 不要把 `data/`、代理数据目录、Cookie、日志、截图或 `.env` 提交到代码仓库；
